...
function villeEstValide(ville) {
    var retour = false;
    var paysList = jaxGetJson("http://localhost/javascript-web-srv/data/villes/villes.json");
...
function afficherAideSaisis(inputElt) {
    ...
    inputElt.addEventListener("input", function (e) {
        ...
        PaysList.forEach(function (Pays) {
            ...
            /* afficher aide */
            for (var j = 0; j < Pays.villes.length; j++) {
                /* si saisie correspond nom ville alors afficher aide-saisie */

                if (Pays.villes[j].toLowerCase().indexOf(e.target.value.toLowerCase()) === 0) {
                    var pElt = document.createElement("p");
                    pElt.textContent = Pays.villes[j] + ", " + Pays.nom;
